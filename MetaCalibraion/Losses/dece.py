...
class Robust_Sigmoid(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, input, sigma=1.0, gpu=False, device=None):
        '''
        :param ctx:
        ...
        torch_half = torch.cuda.FloatTensor(
            [0.5], device=device) if gpu else torch.FloatTensor([0.5])
...
class DECE(nn.Module):
    ...
    def forward(self, input, target):
        ...
        batch_indicators = robust_sigmoid(torch.transpose(batch_pred_diffs, dim0=1, dim1=2), self.t_a, gpu, self.device)
