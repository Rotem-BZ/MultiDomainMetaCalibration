...
from train_utils import train_single_epoch, test_single_epoch, train_single_epoch2
...
def parseArgs():
    ...
    load_loc = ''
    model = "resnet18"
    exp_name = 'md_train_no_meta'
    epoch = 350
    multi_meta = False
    multi_domain_train = True
    multi_domain_meta = False
    first_milestone = int(epoch * 3 / 7)  # Milestone for change in lr
    second_milestone = int(epoch * 5 / 7)  # Milestone for change in lr
    gamma_schedule_step1 = int(epoch * 2 / 7)
    gamma_schedule_step2 = int(epoch * 5 / 7)
    ...
    parser.add_argument("--multi_meta", action="store_true", dest="multi_meta", default=multi_meta,
                        help="whether to use multi_meta")
    parser.add_argument("--reg_type", type=str, default="vector_label_smoothing", dest="reg_type",
                        help='what regularization type to use - scalar_label_smoothing, vector_label_smoothing, l2')
    ...
    parser.add_argument("--multi_domain_meta", action="store_true",
                        help="whether to use multidomain training", default=multi_domain_meta)
    parser.add_argument("--multi_domain_train", action="store_true",
                        default=multi_domain_train)
...
if __name__ == "__main__":
    ...
    print("CUDA set: " + str(cuda) + str(device))
    ...
    if args.meta_calibration:
        ...
        Net.wide_resnet.Wide_ResNet_Cifar.meta = True
    ...
    n_domains = 1
    if args.meta_calibration:
        train_loader, val_loader, meta_loader, n_domains = dataset_loader[args.dataset].get_train_valid_loader(
            batch_size=args.train_batch_size,
            augment=args.data_aug,
            random_seed=1,
            pin_memory=args.gpu,
            meta_val=True,
            multi_domain_train=args.multi_domain_train,
            multi_domain_meta=args.multi_domain_meta,
            multi_meta=args.multi_meta
        )
        print(f'n domains {n_domains}')
    else:
    ...
    with tqdm.tqdm(total=num_epochs) as pbar_epochs:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_func = train_single_epoch2 if args.multi_meta else train_single_epoch
            train_loss, train_ece = train_func(epoch,
                                               net,
                                               train_loader,
                                               meta_loader_copy,
                                               optimizer,
            ...
            _, test_acc, _, _, _, test_ece = test_classification_net(
                net, test_loader, device, return_ece=True)
            _, meta_acc, _, _, _, meta_ece = test_classification_net(
                net, meta_loader_copy, device, return_ece=True)
            ...
            experiment_update_dict = {'train_loss': train_loss,
                                      ...
                                      'meta_ece': meta_ece,
                                      'meta_acc': meta_acc}
            if args.meta_calibration and args.reg_type == 'scalar_label_smoothing':
                experiment_update_dict['learn_reg_vals'] = learnable_regularization[-1].item()
            if args.meta_calibration and args.reg_type == 'vector_label_smoothing':
                experiment_update_dict['learn_reg_vals'] = torch.stack(learnable_regularization).tolist()

            update_json_experiment_log_dict(
                experiment_update_dict, json_experiment_log_file_name)
            # print(learnable_regularization)
