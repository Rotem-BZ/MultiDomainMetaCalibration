...
def calculate_regularized_loss(logits, labels, learnable_regularization, loss_function,
    ...
    if args.reg_type == 'scalar_label_smoothing' or args.reg_type == 'vector_label_smoothing':
...
def calculate_regularized_loss2(logits, labels, domains, learnable_regularization, loss_function,
                               gamma, lamda, device, model_fc, args):
    if args.dataset == 'cifar10':
        num_classes = 10

    soft_labels = create_class_smooth_labels2(
        labels, domains, learnable_regularization, num_classes, device)
...
def create_class_smooth_labels2(labels, domains:torch.Tensor, label_smoothing_list, num_classes, device):
    targets = []
    assert len(domains) == len(labels)
    # print(f'domains {domains.detach().cpu().numpy().tolist()}')
    for target, domain in zip(labels, domains):
        label_smoothing = label_smoothing_list[domain]
        soft_target = torch.zeros(num_classes, device=device)
        soft_target[target] = (1.0 - label_smoothing[target])
        soft_target += label_smoothing[target] / num_classes
        targets.append(soft_target)
    return torch.stack(targets)
...
def train_single_epoch(epoch,
    ...
    for batch_idx, batch in enumerate(loaders):
        if args.meta_calibration:
            ((data, labels, _), (data_val, labels_val)) = batch
...
def train_single_epoch2(epoch,
    ...
    for batch_idx, batch in enumerate(loaders):
        ((data, labels, domains), (data_val, labels_val)) = batch
        data_val = data_val.to(device)
        labels_val = labels_val.to(device)
        ...
        logits = model(data)
        loss = calculate_regularized_loss2(
            logits, labels, domains, learnable_regularization, loss_function, gamma, lamda, device, model_fc, args)
        grad = torch.autograd.grad(loss, fast_parameters, create_graph=True)
        fast_parameters = []
        for k, weight in enumerate(model_fc.parameters()):
            if weight.fast is None:
