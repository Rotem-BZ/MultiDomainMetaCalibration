...
def glass_blur(x, severity=1):
    ...
    c = [(0.05, 1, 1), (0.25, 1, 1), (0.4, 1, 1),
         (0.25, 1, 2), (0.4, 1, 2)][severity - 1]
...
def frost(x, severity=1):
    ...
    x_start, y_start = np.random.randint(
        0, frost.shape[0] - 32), np.random.randint(0, frost.shape[1] - 32)
...
def snow(x, severity=1):
    ...
    x = c[6] * x + (1 - c[6]) * np.maximum(x, cv2.cvtColor(x,
                                                           cv2.COLOR_RGB2GRAY).reshape(32, 32, 1) * 1.5 + 0.5)
...
transforms_dict = collections.OrderedDict()
transforms_dict['Gaussian Noise'] = gaussian_noise
transforms_dict['Shot Noise'] = shot_noise
transforms_dict['Impulse Noise'] = impulse_noise
transforms_dict['Defocus Blur'] = defocus_blur
transforms_dict['Glass Blur'] = glass_blur
transforms_dict['Motion Blur'] = motion_blur
transforms_dict['Zoom Blur'] = zoom_blur
transforms_dict['Snow'] = snow
transforms_dict['Frost'] = frost
transforms_dict['Fog'] = fog
transforms_dict['Brightness'] = brightness
transforms_dict['Contrast'] = contrast
transforms_dict['Elastic'] = elastic_transform
transforms_dict['Pixelate'] = pixelate
transforms_dict['JPEG'] = jpeg_compression

transforms_dict['Speckle Noise'] = speckle_noise
transforms_dict['Gaussian Blur'] = gaussian_blur
transforms_dict['Spatter'] = spatter
transforms_dict['Saturate'] = saturate
