...
class CorruptedCIFAR10(datasets.CIFAR10):
    def __init__(self, return_domain=True, **kwargs):
        super(CorruptedCIFAR10, self).__init__(**kwargs)
        self.return_domain = return_domain
        self.corruption_type_list = ['Gaussian Noise', 'Shot Noise', 'Defocus Blur',
    ...
    def __getitem__(self, index: int):
        ...
        corruption_type_list = ['Gaussian Noise', 'Shot Noise', 'Defocus Blur',
                                ...
                                'Brightness', 'Contrast', 'Pixelate', 'JPEG',
                                'Speckle Noise', 'Gaussian Blur', 'Spatter', 'Saturate']
        corruption_type_idx = np.random.randint(0, len(self.corruption_type_list))
        ...
        corruption_type = self.corruption_type_list[corruption_type_idx]
        ...
        if self.return_domain:
            return img, target, corruption_type_idx
        return img, target
...
def get_train_valid_loader(batch_size,
    ...
    # load the dataset
    data_dir = '/home/niv.ko/data'
    domains = 1
    if multi_domain_train:
        train_dataset = CorruptedCIFAR10(
            root=data_dir, train=True, return_domain=multi_meta,
            download=False, transform=valid_transform)
        domains = len(train_dataset.corruption_type_list) if meta_val and multi_meta else domains
    else:
    ...
    if meta_val:
        if multi_domain_meta:
            meta_dataset = CorruptedCIFAR10(
                root=data_dir, train=True, return_domain=False,
                download=False, transform=valid_transform)
        else:
    ...
    if get_val_temp > 0:
        ...
        return (train_loader, valid_loader, meta_loader, domains)
    else:
